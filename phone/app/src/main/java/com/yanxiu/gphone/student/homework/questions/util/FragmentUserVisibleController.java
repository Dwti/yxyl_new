package com.yanxiu.gphone.student.homework.questions.util;

/**
 * Created by 戴延枫 on 2017/5/8.
 */
...
public class FragmentUserVisibleController {
    ...
    public void resume(boolean fromAnswerCard) {
        if (DEBUG) {
            Log.d(TAG, fragmentName + ": resume, userVisibleHint=" + fragment.getUserVisibleHint());
        }
        if (fragment.getUserVisibleHint() || fromAnswerCard) {
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (fragment.getActivity() != null) {
            ...
            if (isVisibleToUser) {
                ...
                // 隐藏正在显示的复合题的第一个子Fragment--因为滑动回来时，需要显示第一个
                if (childFragmentList != null && childFragmentList.size() > 0) {

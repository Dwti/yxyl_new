        ...
        pullReleasingOA.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (pullStateListener != null) {
                    pullStateListener.onPullReleasing(BaseRefreshLayout.this, height);
                }
            }
        });
        pullReleasingOA.start();
...
//                mFooterLayout.requestLayout();//重绘
                if (pullStateListener != null) {
                    pullStateListener.onLoadMorePullReleasing(BaseRefreshLayout.this, height);
                }
            }
        });
        pullReleasingOA.start();
...
//                    float offsetY = decelerateInterpolator.getInterpolation(dy / (mWaveHeight + mHeadHeight)) * dy;
                        ...
                        if (pullStateListener != null) {
                            pullStateListener.onPulling(BaseRefreshLayout.this, offsetY);
                        }
                    }
                    ...
                    if (mChildView != null) {
                        ...
                        mChildView.setTranslationY(offsetY);
                        if (pullStateListener != null) {
                            pullStateListener.onLoadMorePulling(BaseRefreshLayout.this, -offsetY);
                        }
                    }
                }
                    ...
                    if (pullState == 1) { //下拉刷新
                        if (mChildView.getTranslationY() >= mHeadHeight + mWaveHeight) {
                            ...
                            if (pullToRefreshPullingListener != null) {
                                pullToRefreshPullingListener.onRefresh(BaseRefreshLayout.this);
                            }
                        } else {
                        ...
                        if (-mChildView.getTranslationY() >= mFooterHeight) { //因为是负数
                            ...
                            isLoadMore = true;
                            if (pullToRefreshPullingListener != null) {
                                pullToRefreshPullingListener.onLoadMore(BaseRefreshLayout.this);
                            }
                        } else {
